...
void factura(tyVenta &venta, tyVecProm v[], int &iP, tyVecTel vt[], int &iT, float &impVta, float &impComi){
	
	impVta = venta.qVendida * vt[iT].precio;
	impComi = impVta * v[iP].comision / 100;
	cout << "importe de factura : " << impVta << endl;
	cout << "importe de comision : "<< impComi << " de promotor/a : " << v[iP].nombreProm << endl;
...
void iniVecComi(float v[], int tope){
...
void comisiones(float v[], int &iP, float &comi){
	
	v[iP] += comi;
	
}

void totalComisiones(tyVecProm v[], float comiVec[] ){
	
	cout << "----------------------------" << endl;
	cout <<  "Total de comisiones: " << endl;
	cout << "----------------------------" << endl;
	for(int i = 0; i < TOPEPROM; i++){
		cout << v[i].nombreProm << " comision total : " << comiVec[i] << endl;
	}
	
}

void iniVecAcumTel(int v[], int tope){
...
void telVendidos(int v[], int  &iT, int &q){
